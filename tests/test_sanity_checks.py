...
def _generate_id(env_name: str, case_desc: str, suffix: str = "") -> str:
    safe_desc = "".join(c if c.isalnum() or c == "_" else "_" for c in case_desc).lower()
...
for env_name_param in ["connect4", "nim"]:
    try:
        ...
        for case in _sanity_cases:
            ...
            az_mcts_noload_id = _generate_id(env_name_param, case.description, suffix="-az_mcts_noload")
            AZ_MCTS_PARAMS.append(pytest.param(env_name_param, case, False, id=az_mcts_noload_id))

            # AlphaZero MCTS Parameters (Load)
            az_mcts_load_id = _generate_id(env_name_param, case.description, suffix="-az_mcts_load")
            AZ_MCTS_PARAMS.append(pytest.param(env_name_param, case, True, id=az_mcts_load_id))

            # AlphaZero Network Eval Parameters (Load Only)
            az_net_load_id = _generate_id(env_name_param, case.description, suffix="-az_net_load")
...
class TestSanityChecks:
    ...
    def _run_single_mcts_check(
        ...
        if not root_node or not root_node.children:
            ...
            if legal_actions:
                assert chosen_action is not None, "Agent returned None action when legal moves exist."
                assert chosen_action in legal_actions, "Agent chose an illegal action."
            return

        legal_actions = current_env.get_legal_actions()
        assert chosen_action in legal_actions, f"Chosen action {chosen_action} not in legal actions {legal_actions}"

        if check_case.expected_action is not None:
            assert chosen_action == check_case.expected_action, f"Expected action {check_case.expected_action} but got {chosen_action}"
    ...
    def _run_single_alphazero_mcts_check(
        ...
        if load_attempted:
            if agent.load():
                ...
                print("Warning: Configured to load weights, but no weights file found or load failed. Using initial network weights.")
    ...
    def _run_single_alphazero_network_check(
        ...
        if agent.network is None:
             pytest.skip(f"Cannot run network check for {check_case.description}: Agent network is None.")
             return
        ...
        if agent.config.should_use_network: # Check if load was intended
            if agent.load():
                ...
                print("Warning: Configured to load weights, but no weights file found or load failed. Network predictions might be random.")
        else:
             # This check shouldn't normally run if should_use_network is False, but handle defensively
             print("Info: Running network check with un-trained (initial) network weights.")


        try:
            policy_np, value_np = agent.network.predict(check_case.state)
        except Exception as e:
            raise AssertionError(f"Network prediction failed for state {check_case.state} with error: {e}")
        ...
        if check_case.expected_value is not None:
            ...
            if check_case.expected_value > 0.1:
                assert value_np > 0.0, "Predicted value should be positive for expected win state."
            elif check_case.expected_value < -0.1:
                assert value_np < 0.0, "Predicted value should be negative for expected loss state."
            else:
                assert abs(value_np - 0.0) <= 0.2, "Predicted value should be close to zero for expected draw state."
        else:
        ...
        if not legal_actions:
            ...
            assert check_case.expected_action is None, "Test case has expected_action but no legal actions exist."
            return
        ...
        for action in legal_actions:
            ...
            if idx is not None and 0 <= idx < policy_size and idx < len(policy_np):
                ...
                print(f"  Warning: Failed to map action {action} to valid index (idx={idx}, policy_len={len(policy_np)})")
        ...
        if check_case.expected_action is not None:
            ...
            assert best_predicted_action is not None, "Could not determine best predicted action (likely due to mapping errors or all probs negative)."
            assert best_predicted_action == check_case.expected_action, f"Action with highest predicted probability ({best_predicted_action} with p={action_probs.get(best_predicted_action, -1):.4f}) does not match expected action ({check_case.expected_action})."
        else:
    ...
    def test_mcts_action_sanity(self, env_name: str, check_case: SanityCheckState):
        ...
        agent = MCTSAgent(
            ...
            debug=config.mcts.debug,
        )
        self._run_single_mcts_check(agent, env, check_case)


    @pytest.mark.parametrize("env_name, check_case, load_weights", AZ_MCTS_PARAMS)
    def test_alphazero_mcts_action_sanity(self, env_name: str, check_case: SanityCheckState, load_weights: bool):
        """Runs AlphaZero MCTS action selection sanity checks (with and without loading weights)."""
        config = self._get_config(env_name)
        config.alpha_zero.should_use_network = load_weights # Configure agent based on parameter
        env = get_environment(config.env)
    ...
    def test_alphazero_network_output_sanity(self, env_name: str, check_case: SanityCheckState):
        """Runs AlphaZero direct network output sanity checks (assumes trying to load weights)."""
        config = self._get_config(env_name)
        config.alpha_zero.should_use_network = True # Always try to load for network eval check
        env = get_environment(config.env)
        # Need training_config for agent init, even if not used in this check
        agent = AlphaZeroAgent(env, config.alpha_zero, config.training)
